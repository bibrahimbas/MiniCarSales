...
const initializeProp = (type, config, value = '', validationRules, isValid = true) => {
    return { key: config.label, elementType: type, elementConfig: config, value: value, validation: validationRules, valid: isValid, touched: false };
}
            ...
            model: initializeProp('select',
                {
                    dataFunc: function () {
                        ...
                        return getCacheData("MakeModel")
                            .then(response => {
                                if (response.data) {
                                    var selectedMake = response.data.filter(make => make.make === state.carForm.make.value);
